...
| `npm run deploy`     | Check + build + deploy to Workers    |
...
5. `npx wrangler login` once, then `npm run deploy` — it runs `npm run check` (type-check, lint, tests) before building and deploying (or connect the repo via Workers Builds — that path must run `npm run deploy` too, since the committed `CACHE_VERSION` is only regenerated by the build)

|                | Without KV   | With KV                 |
| -------------- | ------------ | ----------------------- |
| Data           | Memory cache | KV cache (30m/1h/2h/6h) |
