...
import { UPSTREAM_FETCH_OPTS, upstreamConfig, upstreamEndpoints, upstreamUrl } from "@/server/config";
...
export async function fetchDailyPapersItems(ctx: AppContext): Promise<NewsItem[]> {
  const url = upstreamUrl(upstreamConfig.huggingfaceSite, upstreamEndpoints.hfDailyPapers);
  const raw = await ctx.http.json<unknown>(url, UPSTREAM_FETCH_OPTS);
  return requireParsed(parseDailyPapers(raw));
}
