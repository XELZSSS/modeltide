...
const LITELLM_URL = `${upstreamConfig.githubRaw}${upstreamEndpoints.litellmPricing}`;
...
describe("getOfficialPricing", () => {
  ...
  it("reads the branch-tracking LiteLLM table with the long-timeout fetch options", async () => {
    const { ctx, calls } = pricingCtx(litellmTable());
    await getOfficialPricing(ctx);
    expect(calls[0]?.url).toBe(LITELLM_URL);
    expect(calls[0]?.init).toMatchObject({ timeoutMs: 15_000, retries: 1 });
  });

  it("caches the payload so the second call skips upstream", async () => {
    const { ctx, calls } = pricingCtx(litellmTable());
    const first = await getOfficialPricing(ctx);
    const second = await getOfficialPricing(ctx);
