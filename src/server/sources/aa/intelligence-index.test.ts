...
import { getIntelligenceIndex, getIntelligenceIndexResult } from "@/server/sources/aa/intelligence-index";
import { ttlFor } from "@/shared/config";

vi.mock("@/server/sources/aa/fetch", async (importOriginal) => {
  const mod = await importOriginal<typeof import("@/server/sources/aa/fetch")>();
...
describe("getIntelligenceIndex", () => {
  ...
  it("shortens the cached ttl for a partial result", async () => {
    vi.mocked(fetchAndParseEnrich)
      .mockReset()
      .mockResolvedValue([] as never);
    const { ctx, kvStore } = testCtx(new Map<string, string>());
    await getIntelligenceIndex(ctx);
    const envelope = JSON.parse(kvStore.get("v-test:artificial-analysis-index") ?? "{}") as { t?: number };
    expect(envelope.t).toBeGreaterThan(0);
    expect(envelope.t!).toBeLessThanOrEqual(ttlFor(true));
  });
