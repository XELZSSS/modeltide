...
打开 `http://localhost:3000` —— 开发服务器会同时运行 React 前端与 Worker API，并内置本地 KV 缓存模拟，无需额外配置即可启动
...
npm run build      # 生产构建（同步更新 CSP 哈希）
...
单个 Cloudflare Worker 同时托管静态资源与 API；上游数据缓存在 KV 命名空间，由定时 Cron 刷新

1. **Fork** 本仓库
2. 在 [Cloudflare Dashboard](https://dash.cloudflare.com) → **Workers & Pages** → **KV** 创建一个命名空间，复制其 ID
3. **配置** —— 将 KV 命名空间 ID 填入 `wrangler.jsonc` 的 `id` 字段并提交
4. **部署** —— 在 **Workers & Pages** → **Create application** → **Import a repository** 中选择 Fork 的仓库，点击 **Save and Deploy**
5. **自动更新** —— 每次推送到仓库都会自动重新构建并部署
